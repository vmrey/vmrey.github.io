...
function renderSidebar({
  ...
  let navCategoriesHtml = `        <!-- 导航分组 1：分类专栏 (支持多层级子专栏) -->
        <div class="sidebar-nav-group">
          ...
          <!-- 全部文章 -->
          <${isLinkMode ? 'a href="' + homeUrl + '"' : 'button type="button"'} class="sidebar-nav-item ${isLinkMode ? '' : 'active'} category-filter-btn" data-tag="all" data-label="全部文章">
            ...
            <span class="nav-badge">${categoryStats.all || 0}</span>
          </${isLinkMode ? 'a' : 'button'}>
`;
  ...
  blogConfig.categories.forEach(cat => {
    ...
    if (validChildren.length === 0) {
      navCategoriesHtml += `
          <!-- 专栏：${cat.name} -->
          <${isLinkMode ? 'a href="' + catLinkHref + '"' : 'button type="button"'} class="sidebar-nav-item category-filter-btn" data-tag="${cat.tag}" data-label="${cat.name}">
            <span class="nav-item-left">
            ...
            <span class="nav-badge">${parentCount}</span>
          </${isLinkMode ? 'a' : 'button'}>
`;
    } else {
      let childrenHtml = validChildren.map(sub => {
        ...
        return `              <${isLinkMode ? 'a href="' + subLinkHref + '"' : 'button type="button"'} class="subtree-item category-filter-btn" data-tag="${sub.tag}" data-label="${sub.name}">
                <span class="subtree-item-left">
                  <span class="subtree-item-dot"></span>
                  <span class="subtree-item-name">${sub.name}</span>
                </span>
                <span class="subtree-badge">${subCount}</span>
              </${isLinkMode ? 'a' : 'button'}>`;
      }).join('\n');
      ...
      navCategoriesHtml += `
          ...
          <div class="nav-item-tree open">
            <div class="tree-header-row">
              ...
              <button type="button" class="subtree-toggle-btn" aria-label="展开或折叠${cat.name}子菜单">
...
${navCategoriesHtml}
        ...
        <div class="sidebar-nav-group">
          ...
          <a href="${filesUrl}" class="sidebar-nav-item ${activePage === 'files' ? 'active' : ''}">
            <span class="nav-item-left">
            ...
            <span class="nav-badge">${resourceFilesCount}</span>
          </a>
          <a href="${aboutUrl}" class="sidebar-nav-item ${activePage === 'about' ? 'active' : ''}">
            <span class="nav-item-left">
          ...
          <a href="${blogConfig.githubUrl}" target="_blank" rel="noopener noreferrer" class="sidebar-nav-item">
            <span class="nav-item-left">
