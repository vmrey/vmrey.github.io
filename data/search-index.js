...
window.SEARCH_DATABASE = window.BLOG_SEARCH_INDEX = [
  ...
  },
  {
    ...
    "content": "Cloudflare DDNS 动态域名解析自动更新脚本与使用指南 在家庭宽带、NAS 或无固定公网 IP 的服务器环境中，公网 IP 经常会因宽带重拨或运营商策略而发生变动。通过 Cloudflare 提供的 DNS API，我们可以利用轻量 Shell 脚本实时检测本机外网 IP，并在 IP 发生变化时自动同步更新到 Cloudflare DNS 解析记录中。 本文介绍的 cf-ddns.sh 脚本具备 智能本地缓存 、 IPv4/IPv6 双栈支持 与 命令行参数覆盖 等特性，适用于 CentOS、Ubuntu、Debian、macOS 等绝大多数 Unix-like 环境。 --- 🌟 核心特性 - 双栈支持 ：支持 IPv4（A 记录）与 IPv6（AAAA 记录）自动探测与同步； - 智能缓存 ：本地维护 IP 与 Zone/Record ID 缓存，仅在 IP 发生真实变动时调用 Cloudflare API，杜绝无效请求与频率限制； - 安全认证 ：采用 Cloudflare 官方推荐的 API Token （Bearer 模式），按最小权限原则配置； - 灵活调用 ：支持修改脚本静态配置，或直接通过命令行标志传参（ -k , -z , -h , -t , -f , -p ）； - 全自动创建/更新 ：目标 DNS 记录不存在时自动调用 POST 创建，已存在时自动执行 PUT 更新。 --- 📥 1. 下载与安装脚本 通过本站托管的资源一键下载脚本并赋予可执行权限： 方式一：使用 curl 下载（推荐） bash curl -Lo cf-ddns.sh https://vmrey.github.io/assets/files/cf-ddns.sh && chmod +x cf-ddns.sh 方式二：使用 wget 下载 bash wget -O cf-ddns.sh https://vmrey.github.io/assets/files/cf-ddns.sh && chmod +x cf-ddns.sh <div class=\"nested-file-row standalone-file-card\"> <div class=\"file-row-left\"> <div class=\"file-type-pill\" style=\"color: a855f7; border-color: a855f740; background: rgba 168, 85, 247, 0.1 ;\"> .SH </div> <div class=\"file-name-meta\"> <div class=\"file-name-line\"> <span class=\"file-main-name\">cf-ddns.sh</span> <span class=\"file-size-tag\">Shell 脚本</span> </div> <div class=\"file-desc-line\" title=\"Cloudflare DDNS 动态域名解析自动检测与同步脚本\">Cloudflare DDNS 动态域名解析自动检测与同步脚本（支持 IPv4/IPv6、智能缓存与 Crontab）</div> </div> </div> <div class=\"file-row-actions\"> <a href=\"../assets/files/cf-ddns.sh\" download=\"cf-ddns.sh\" class=\"file-action-btn primary\" title=\"直接下载 cf-ddns.sh\"> <svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4\"></path><polyline points=\"7 10 12 15 17 10\"></polyline><line x1=\"12\" y1=\"15\" x2=\"12\" y2=\"3\"></line></svg> <span>直接下载</span> </a> <a href=\"../files.html\" class=\"file-action-btn\" title=\"前往全站文件中心在线预览与管理\"> <svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z\"></path><circle cx=\"12\" cy=\"12\" r=\"3\"></circle></svg> <span>文件中心</span> </a> </div> </div> --- ⚙️ 2. 配置说明 步骤 A：获取 Cloudflare API Token 1. 登录 Cloudflare 控制台，进入 API 令牌管理页面 https://dash.cloudflare.com/profile/api-tokens ； 2. 点击 创建令牌 Create Token ； 3. 选择 编辑区域 DNS Edit zone DNS 模板； 4. 在「区域资源 Zone Resources 」中选择你的目标域名（如 example.com ）； 5. 点击提交并复制生成的 Token 密钥字符串。 --- 步骤 B：修改脚本配置 使用编辑器打开 cf-ddns.sh ： bash vi cf-ddns.sh 修改脚本顶部的基础配置项： bash ---------------------------------------------------- 1. 配置项 Configuration ---------------------------------------------------- Cloudflare 的 API Token【❗推荐使用，权限更安全】 CF_TOKEN=\"your_cloudflare_api_token_here\" Cloudflare 的顶级域名（主域名） CFZONE_NAME=\"example.com\" 需要进行 DDNS 解析的目标域名（可填子域名或根域名） CFRECORD_NAME=\"home.example.com\" 记录类型：A IPv4 或 AAAA IPv6 ，默认 A CFRECORD_TYPE=\"A\" Cloudflare TTL 存活时间，单位秒，120 ~ 86400，120 为 2 分钟 CFTTL=120 是否开启 Cloudflare CDN 代理：true 开启橙云 | false 仅 DNS 解析/灰云 CFPROXIED=false 强制更新开关：false 默认 | true 忽略本地 IP 缓存强制推送到 CF FORCE=false --- 🚀 3. 运行与手动测试 方式一：直接运行（读取脚本内配置） bash ./cf-ddns.sh 方式二：通过命令行参数传入（无需修改脚本文件） 脚本内置了 getopts 参数解析器，非常适合在多域名批量同步场景或容器化任务中调用： bash ./cf-ddns.sh -k \"your_api_token\" -z \"example.com\" -h \"home.example.com\" -t A -p false 命令行参数对照表 | 参数 | 说明 | 示例值 | | :--- | :--- | :--- | | -k | Cloudflare API Token 密钥 | your_token_str | | -z | 托管在 CF 上的顶级主域名 | example.com | | -h | 需要绑定的 DDNS 完整域名 | home.example.com | | -t | 解析记录类型 A 或 AAAA | A IPv4 / AAAA IPv6 | | -p | 是否开启 CDN 代理加速 true / false | false | | -f | 是否强制刷新 DNS（忽略 IP 缓存比对） | false | --- ⏰ 4. 设置 Crontab 定时自动守护 配置系统定时任务，让脚本每隔 2 分钟或 5 分钟在后台自动巡检一次 IP： bash crontab -e 在打开的定时任务文件中追加以下任一规则： 规则 A：静默巡检（无变动不产生多余日志） cron 每 2 分钟静默检测一次 /2 /path/to/cf-ddns.sh >/dev/null 2>&1 规则 B：记录执行日志（便于故障排查） cron 每 5 分钟检测一次并追加日志到 /var/log/cf-ddns.log /5 /path/to/cf-ddns.sh >> /var/log/cf-ddns.log 2>&1 > 注意 ：请将 /path/to/cf-ddns.sh 替换为您服务器上的绝对路径（例如 /root/scripts/cf-ddns.sh ）。 重载/重启 Cron 服务（根据 Linux 发行版选择）： bash CentOS / RHEL systemctl reload crond.service Ubuntu / Debian systemctl restart cron --- 📁 5. 缓存与工作机制剖析 为降低对 Cloudflare API 的请求频次并提升脚本执行速度，脚本在首次运行后会在当前用户的家目录（ $HOME ）下维护两份轻量缓存文件： 1. ~/.cf-wan_ip_<域名>.txt ： - 记录上一次成功同步的公网 IP； - 每次运行时先通过 https://ipv4.icanhazip.com 获取当前外网 IP，若与缓存一致且未指定 -f 参数，脚本将立即退出，避免不必要的网络开销。 2. ~/.cf-id_<域名>.txt ： - 缓存对应域名的 Zone ID 与 Record ID ； - 避免每次更新时重复调用 API 查询区域 ID 与解析记录列表。 > 💡 小提示 ：如遇到更换域名、重置 DNS 记录或切换 API Token 后解析不生效的情况，只需执行 rm -f ~/.cf-id_ .txt ~/.cf-wan_ip_ .txt 清理本地缓存即可。",
    "sections": [
      {
        "title": "🌟 核心特性",
        "anchor": "#核心特性",
      ...
      {
        "title": "方式一：使用 `curl` 下载（推荐）",
        "anchor": "#方式一-使用-curl-下载-推荐",
        "id": "方式一-使用-curl-下载-推荐"
      },
      {
        "title": "方式二：使用 `wget` 下载",
        "anchor": "#方式二-使用-wget-下载",
        "id": "方式二-使用-wget-下载"
      },
      {
        "title": "⚙️ 2. 配置说明",
      ...
      {
        "title": "步骤 A：获取 Cloudflare API Token",
        "anchor": "#步骤-a-获取-cloudflare-api-token",
        "id": "步骤-a-获取-cloudflare-api-token"
      },
      {
        "title": "步骤 B：修改脚本配置",
        "anchor": "#步骤-b-修改脚本配置",
      ...
      {
        "title": "方式一：直接运行（读取脚本内配置）",
        "anchor": "#方式一-直接运行-读取脚本内配置",
        "id": "方式一-直接运行-读取脚本内配置"
      },
      {
        "title": "方式二：通过命令行参数传入（无需修改脚本文件）",
        "anchor": "#方式二-通过命令行参数传入-无需修改脚本文件",
        "id": "方式二-通过命令行参数传入-无需修改脚本文件"
      },
      {
        "title": "命令行参数对照表",
        "anchor": "#命令行参数对照表",
      ...
      {
        "title": "⏰ 4. 设置 Crontab 定时自动守护",
        "anchor": "#4-设置-crontab-定时自动守护",
        "id": "4-设置-crontab-定时自动守护"
      },
      {
        "title": "规则 A：静默巡检（无变动不产生多余日志）",
        "anchor": "#规则-a-静默巡检-无变动不产生多余日志",
        "id": "规则-a-静默巡检-无变动不产生多余日志"
      },
      {
        "title": "规则 B：记录执行日志（便于故障排查）",
        "anchor": "#规则-b-记录执行日志-便于故障排查",
        "id": "规则-b-记录执行日志-便于故障排查"
      },
      {
        "title": "📁 5. 缓存与工作机制剖析",
        "anchor": "#5-缓存与工作机制剖析",
  ...
  },
  {
    ...
    "content": "Xray 一键安装脚本使用说明 本文档提供了运行托管在 GitHub 上的 xray.sh 脚本的通用安装命令，并说明了具体的使用方法及相关注意事项。 1. 安装与脚本下载 <div class=\"nested-file-row standalone-file-card\"> <div class=\"file-row-left\"> <div class=\"file-type-pill\" style=\"color: a855f7; border-color: a855f740; background: rgba 168, 85, 247, 0.1 ;\"> .SH </div> <div class=\"file-name-meta\"> <div class=\"file-name-line\"> <span class=\"file-main-name\">xray.sh</span> <span class=\"file-size-tag\">Shell 脚本</span> </div> <div class=\"file-desc-line\" title=\"Xray Core 核心网络代理服务一键安装与证书部署脚本\">Xray Core 核心网络代理服务一键安装与证书部署脚本</div> </div> </div> <div class=\"file-row-actions\"> <a href=\"../assets/files/xray.sh\" download=\"xray.sh\" class=\"file-action-btn primary\" title=\"直接下载 xray.sh\"> <svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4\"></path><polyline points=\"7 10 12 15 17 10\"></polyline><line x1=\"12\" y1=\"15\" x2=\"12\" y2=\"3\"></line></svg> <span>直接下载</span> </a> <a href=\"../files.html\" class=\"file-action-btn\" title=\"前往全站文件中心在线预览与管理\"> <svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z\"></path><circle cx=\"12\" cy=\"12\" r=\"3\"></circle></svg> <span>文件中心</span> </a> </div> </div> 您可以根据服务器的环境（是否预装了 curl 或 wget ），选择以下任意一种方式进行安装： 方法一：使用 curl （推荐，最快捷） 此命令会直接读取网络文件并执行，不会在服务器本地留下脚本文件。 bash bash < curl -Ls https://vmrey.github.io/assets/files/xray.sh 方法二：使用 wget 如果您的服务器没有安装 curl ，可以使用 wget 达到相同的效果。 bash wget -O- https://vmrey.github.io/assets/files/xray.sh | bash 方法三：分步执行（适合需要先检查代码的用户） 将脚本下载到本地，赋予执行权限后再手动运行。 bash 1. 下载脚本 curl -O https://vmrey.github.io/assets/files/xray.sh 2. 赋予脚本执行权限 chmod +x xray.sh 3. 运行脚本 ./xray.sh --- 2. 怎么用？（使用步骤） 1. 连接服务器 ：使用 SSH 客户端（如 Termius, Xshell, PuTTY 或 macOS/Linux 自带的终端）连接到您的 Linux 服务器（VPS）。 2. 复制命令 ：复制上述“安装命令”中的任意一条。 3. 执行安装 ：在服务器终端内粘贴该命令并按回车键运行。 4. 跟随提示操作 ：脚本运行后，通常会弹出交互式菜单或按步骤提示您输入/确认相关配置（如选择安装的协议、端口号、伪装域名等）。请仔细阅读终端打印的提示，输入对应数字或按回车确认。 5. 保存节点信息 ：安装完成后，脚本一般会在终端底部输出最终的客户端连接信息（如 VLESS/VMess 分享链接、配置 JSON 或二维码），请务必妥善复制并保存这些信息，用于配置您的本地客户端。 ---",
    "sections": [
      {
        "title": "1. 安装与脚本下载",
        "anchor": "#1-安装与脚本下载",
  ...
  },
  {
    ...
    "content": "轻量级高性能内网穿透：frp 远程桌面 RDP 搭建与全平台配置实战 🌐 一、架构原理与穿透模式选型 在远程办公或跨网络运维场景中，Windows 原生远程桌面协议（RDP, 默认端口 3389 ）凭借极低的操作延迟、原生剪贴板互通以及无缝音频/磁盘重定向，体验远优于常规第三方远程控制软件。然而，绝大多数家庭宽带与公司内网均处于 NAT 局域网之后，缺乏公网 IP。 frp Fast Reverse Proxy 是一款采用 Go 语言开发的高性能反向代理应用，能够将内网服务通过拥有公网 IP 的 VPS 服务器安全暴露至公网。 mermaid flowchart LR subgraph Client \"💻 访问端电脑 Visitor \" mstsc \"mstsc 远程桌面<br/>连接 127.0.0.1:33890\" frpc_v \"frpc 访问端 Visitor 模式 \" mstsc --> frpc_v end subgraph VPS \"☁️ 公网 VPS frps 中继服务端 \" frps \"frps 服务端<br/>Bind Port: 7000\" end subgraph Target \"🖥️ 被控端电脑 Target \" frpc_t \"frpc 被控端 STCP 模式 \" rdp \"Windows RDP 引擎<br/>Local Port: 3389\" frpc_t --> rdp end frpc_v --\"双向 Secret 加密握手\"--> frps frps --\"流量安全中继\"--> frpc_t --- 📊 核心穿透模式技术对比矩阵 | 模式 | 传输协议 | 公网暴露端口 | 安全性 | 适用场景 | | :--- | :--- | :--- | :--- | :--- | | STCP 安全 TCP | 端到端加密 TCP | 仅需服务端通信端口 | 🟢 极高 （无公网暴露，双向密钥鉴权） | 强烈推荐 ：个人与公司核心远程桌面 | | TCP 直接映射 | 标准 TCP 转发 | 需要额外暴露公网端口 | 🟡 中等 （暴露端口易遭受暴力破解扫描） | 临时调试、无法在访问端安装 frpc 的场景 | | XTCP P2P 穿透 | UDP 打洞直连 | 仅握手时使用公网端口 | 🟢 极高 （打洞成功后流量不消耗 VPS 带宽） | 大文件传输、双方 NAT 拓扑简单的网络环境 | > !TIP > 强烈推荐使用 STCP Secret TCP 模式！该模式下 VPS 无需对外开放远程桌面端口，任何扫描工具均无法探测到你的 3389 服务，必须在访问端同时运行带有相同密钥（ sk ）的 frpc visitor 才能建立隧道。 --- 🚀 二、VPS 服务端部署步骤 CentOS / Debian / Ubuntu 方案 A：使用一键自动化部署脚本（最简推荐） <div class=\"nested-file-row standalone-file-card\"> <div class=\"file-row-left\"> <div class=\"file-type-pill\" style=\"color: a855f7; border-color: a855f740; background: rgba 168, 85, 247, 0.1 ;\"> .SH </div> <div class=\"file-name-meta\"> <div class=\"file-name-line\"> <span class=\"file-main-name\">frps.sh</span> <span class=\"file-size-tag\">Shell 脚本</span> </div> <div class=\"file-desc-line\" title=\"FRP 服务端 Linux 一键自动化安装、配置与 Systemd 管理脚本\">FRP 服务端 Linux 一键自动化安装、配置与 Systemd 管理脚本</div> </div> </div> <div class=\"file-row-actions\"> <a href=\"../assets/files/frps.sh\" download=\"frps.sh\" class=\"file-action-btn primary\" title=\"直接下载 frps.sh\"> <svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4\"></path><polyline points=\"7 10 12 15 17 10\"></polyline><line x1=\"12\" y1=\"15\" x2=\"12\" y2=\"3\"></line></svg> <span>直接下载</span> </a> <a href=\"../files.html\" class=\"file-action-btn\" title=\"前往全站文件中心在线预览与管理\"> <svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z\"></path><circle cx=\"12\" cy=\"12\" r=\"3\"></circle></svg> <span>文件中心</span> </a> </div> </div> bash 1. 下载一键脚本并赋予执行权限 wget https://raw.githubusercontent.com/mvscode/frps-onekey/master/frps.sh -O frps.sh 或直接从本站下载: wget https://vmrey.github.io/assets/files/frps.sh -O frps.sh chmod +x frps.sh 2. 运行安装向导 ./frps.sh --- 方案 B：手动二进制部署与 Systemd 守护配置 1. 下载并解压最新发行包 前往 frp GitHub Releases https://github.com/fatedier/frp/releases 获取适用于 Linux 的最新二进制文件： bash 创建程序目录 mkdir -p /usr/local/frps && cd /usr/local/frps 下载对应架构压缩包 以 amd64 为例 wget https://github.com/fatedier/frp/releases/download/v0.58.1/frp_0.58.1_linux_amd64.tar.gz 解压并整理核心文件 tar -xzvf frp_0.58.1_linux_amd64.tar.gz cp frp_0.58.1_linux_amd64/frps ./ cp frp_0.58.1_linux_amd64/frps.toml ./ 2. 编写服务端配置文件 frps.toml / frps.ini - 现代 TOML 规范 frps.toml - 推荐 v0.52+ 版本 ： toml frps.toml bindPort = 7000 身份验证令牌（请务必替换为高强度随机字符串） auth.method = \"token\" auth.token = \"YourSuperStrongSecretToken_2026\" 仪表盘管理后台（可选） webServer.addr = \"0.0.0.0\" webServer.port = 7500 webServer.user = \"admin\" webServer.password = \"Admin_Dashboard_Password_888\" 传输层配置 transport.tcpMux = true transport.maxPoolCount = 5 - 经典 INI 规范 frps.ini - 兼容旧版 ： ini frps.ini common bind_port = 7000 token = YourSuperStrongSecretToken_2026 dashboard_port = 7500 dashboard_user = admin dashboard_pwd = Admin_Dashboard_Password_888 tcp_mux = true 3. 配置 Systemd 系统守护进程 创建 Systemd 服务单元，实现进程异常退出自动拉起与开机自启： bash cat <<EOF > /etc/systemd/system/frps.service Unit Description=Frp Server Service After=network.target syslog.target Wants=network.target Service Type=simple ExecStart=/usr/local/frps/frps -c /usr/local/frps/frps.toml Restart=always RestartSec=5s LimitNOFILE=1048576 Install WantedBy=multi-user.target EOF 4. 启动服务与防火墙放行 bash 重载守护进程并设置开机自启 systemctl daemon-reload systemctl enable --now frps 查看运行状态 systemctl status frps 防火墙端口放行 以 UFW / Firewalld 为例 UFW Debian / Ubuntu : ufw allow 7000/tcp ufw allow 7500/tcp ufw reload Firewalld CentOS / RHEL : firewall-cmd --permanent --zone=public --add-port=7000/tcp firewall-cmd --permanent --zone=public --add-port=7500/tcp firewall-cmd --reload > !IMPORTANT > 如果 VPS 托管在阿里云、腾讯云、华为云或 AWS，必须同时在 云控制台的安全组规则 Security Group 中放行 TCP 7000 与 7500 端口，否则外部无法建立连接！ --- 🖥️ 三、被控端 Windows 客户端配置 Target 1. 开启 Windows 远程桌面功能 1. 进入 Windows「设置」➔「系统」➔「远程桌面」； 2. 打开 「启用远程桌面」 开关； 3. 点击「高级设置」，确认启用「网络级别身份验证 NLA 」。 --- 2. 配置被控端 frpc 下载 Windows 版本的 frp 压缩包，解压至 C:\\frp\\ 目录，编写配置文件： - 现代 TOML 规范 frpc.toml ： toml C:\\frp\\frpc.toml serverAddr = \"1.2.3.4\" 替换为你的 VPS 公网 IP serverPort = 7000 auth.method = \"token\" auth.token = \"YourSuperStrongSecretToken_2026\" STCP 安全加密代理 proxies name = \"rdp_company_pc\" type = \"stcp\" secretKey = \"YourCustomSecretKeyForRDP_999\" localIP = \"127.0.0.1\" localPort = 3389 transport.useEncryption = true transport.useCompression = true - 经典 INI 规范 frpc.ini ： ini C:\\frp\\frpc.ini common server_addr = 1.2.3.4 server_port = 7000 token = YourSuperStrongSecretToken_2026 rdp_company_pc type = stcp sk = YourCustomSecretKeyForRDP_999 local_ip = 127.0.0.1 local_port = 3389 use_encryption = true use_compression = true --- 3. 使用 NSSM 将 frpc 注册为 Windows 系统服务（开机静默自启） 避免命令行黑窗口被误关闭，推荐使用 NSSM Non-Sucking Service Manager https://nssm.cc/ 注册为后台常驻系统服务： cmd :: 1. 以管理员身份打开 CMD 或 PowerShell :: 2. 进入 nssm 所在目录，安装服务 nssm.exe install frpc \"C:\\frp\\frpc.exe\" \"-c C:\\frp\\frpc.toml\" :: 3. 启动并配置开机自启 nssm.exe start frpc nssm.exe set frpc AppStdout \"C:\\frp\\frpc_out.log\" nssm.exe set frpc AppStderr \"C:\\frp\\frpc_err.log\" --- 💻 四、访问端 Windows 配置与极速连接 Visitor 在随身携带的笔记本或外出使用的电脑上，同样解压 frp 并配置 visitor 访问者模式： 1. 编写访问端配置文件 - 现代 TOML 规范 frpc.toml ： toml 访问端 frpc.toml serverAddr = \"1.2.3.4\" 替换为你的 VPS 公网 IP serverPort = 7000 auth.method = \"token\" auth.token = \"YourSuperStrongSecretToken_2026\" STCP Visitor 访问者模式 visitors name = \"rdp_visitor\" type = \"stcp\" serverName = \"rdp_company_pc\" 必须与被控端的代理名称一致 secretKey = \"YourCustomSecretKeyForRDP_999\" 必须与被控端 secretKey 一致 bindAddr = \"127.0.0.1\" bindPort = 33890 本地监听端口 transport.useEncryption = true transport.useCompression = true - 经典 INI 规范 frpc.ini ： ini 访问端 frpc.ini common server_addr = 1.2.3.4 server_port = 7000 token = YourSuperStrongSecretToken_2026 rdp_visitor type = stcp role = visitor server_name = rdp_company_pc sk = YourCustomSecretKeyForRDP_999 bind_addr = 127.0.0.1 bind_port = 33890 use_encryption = true use_compression = true --- 2. 制作一键快速连接批处理脚本 Start-RDP.bat 在桌面新建一个 Start-RDP.bat ，双击即可一键在后台静默启动 visitor 并自动唤起 Windows 远程桌面： bat @echo off title RDP 快速直连通道 cd /d %~dp0 echo 正在建立与远程桌面的安全加密穿透隧道... start /b frpc.exe -c frpc.toml :: 等待隧道握手建立 timeout /t 2 /nobreak >nul echo 正在启动 Windows 远程桌面... start mstsc.exe /v:127.0.0.1:33890 exit --- ⚡ 五、Windows RDP 画面与网络性能极致调优 为了获得接近本地主机的 60FPS 丝滑远程操作体验，建议在 被控端 Windows 进行以下系统级优化： 1. 开启 RemoteFX 与硬件图形加速 组策略 1. 按下 Win + R 输入 gpedit.msc 打开本地组策略编辑器； 2. 依次展开： 计算机配置 ➔ 管理模板 ➔ Windows 组件 ➔ 远程桌面服务 ➔ 远程桌面会话主机 ➔ 远程会话环境 ； 3. 将以下项设置为 「已启用」 ： - 将硬件图形适配器应用于所有远程桌面服务会话 - 为远程桌面连接配置 H.264/AVC 硬件编码 - 优先使用 UDP 网络协议 2. 注册表优化 TCP 连接吞吐 以管理员身份运行 CMD，执行以下优化指令： cmd :: 开启 TCP 窗口缩放与复合拥塞控制 netsh int tcp set global autotuninglevel=normal netsh int tcp set global congestionprovider=ctcp --- 🛠️ 六、高频踩坑与排障自检清单 > !WARNING > 排查连接故障时，请遵循以下自检链条： 1. connect to server error 报错 ： - 检查 VPS 上 frps 进程是否正在运行 systemctl status frps ； - 检查 VPS 云服务商控制台安全组是否放行了 TCP 7000 端口； - 检查被控端与服务端配置中的 token 是否完全一致。 2. authorization failed for visitor 报错 ： - 检查访问端 secretKey 或 sk 是否与被控端完全一致； - 检查访问端的 serverName 是否与被控端代理名 name 精确对齐。 3. 被控端休眠导致连接中断 ： - 在被控端 Windows「电源和睡眠」设置中，将「睡眠状态」设置为 「从不」 ； - 在设备管理器中找到网卡属性，取消勾选「允许计算机关闭此设备以节约电源」。",
    "sections": [
      {
        "title": "🌐 一、架构原理与穿透模式选型",
        "anchor": "#一-架构原理与穿透模式选型",
        "id": "一-架构原理与穿透模式选型"
      },
      {
        "title": "📊 核心穿透模式技术对比矩阵",
      ...
      {
        "title": "方案 A：使用一键自动化部署脚本（最简推荐）",
        "anchor": "#方案-a-使用一键自动化部署脚本-最简推荐",
        "id": "方案-a-使用一键自动化部署脚本-最简推荐"
      },
      {
        "title": "方案 B：手动二进制部署与 Systemd 守护配置",
        "anchor": "#方案-b-手动二进制部署与-systemd-守护配置",
        "id": "方案-b-手动二进制部署与-systemd-守护配置"
      },
      {
        "title": "1. 下载并解压最新发行包",
        "anchor": "#1-下载并解压最新发行包",
      ...
      {
        "title": "2. 编写服务端配置文件 (`frps.toml` / `frps.ini`)",
        "anchor": "#2-编写服务端配置文件-frps-toml-frps-ini",
        "id": "2-编写服务端配置文件-frps-toml-frps-ini"
      },
      {
        "title": "3. 配置 Systemd 系统守护进程",
        "anchor": "#3-配置-systemd-系统守护进程",
        "id": "3-配置-systemd-系统守护进程"
      },
      {
        "title": "4. 启动服务与防火墙放行",
        "anchor": "#4-启动服务与防火墙放行",
      ...
      {
        "title": "🖥️ 三、被控端 Windows 客户端配置 (Target)",
        "anchor": "#三-被控端-windows-客户端配置-target",
        "id": "三-被控端-windows-客户端配置-target"
      },
      {
        "title": "1. 开启 Windows 远程桌面功能",
        "anchor": "#1-开启-windows-远程桌面功能",
        "id": "1-开启-windows-远程桌面功能"
      },
      {
        "title": "2. 配置被控端 `frpc`",
        "anchor": "#2-配置被控端-frpc",
      ...
      {
        "title": "3. 使用 NSSM 将 frpc 注册为 Windows 系统服务（开机静默自启）",
        "anchor": "#3-使用-nssm-将-frpc-注册为-windows-系统服务-开机静默自启",
        "id": "3-使用-nssm-将-frpc-注册为-windows-系统服务-开机静默自启"
      },
      {
        "title": "💻 四、访问端 Windows 配置与极速连接 (Visitor)",
        "anchor": "#四-访问端-windows-配置与极速连接-visitor",
        "id": "四-访问端-windows-配置与极速连接-visitor"
      },
      {
        "title": "1. 编写访问端配置文件",
        "anchor": "#1-编写访问端配置文件",
      ...
      {
        "title": "2. 制作一键快速连接批处理脚本 (`Start-RDP.bat`)",
        "anchor": "#2-制作一键快速连接批处理脚本-start-rdp-bat",
        "id": "2-制作一键快速连接批处理脚本-start-rdp-bat"
      },
      {
        "title": "⚡ 五、Windows RDP 画面与网络性能极致调优",
        "anchor": "#五-windows-rdp-画面与网络性能极致调优",
        "id": "五-windows-rdp-画面与网络性能极致调优"
      },
      {
        "title": "1. 开启 RemoteFX 与硬件图形加速 (组策略)",
        "anchor": "#1-开启-remotefx-与硬件图形加速-组策略",
        "id": "1-开启-remotefx-与硬件图形加速-组策略"
      },
      {
        "title": "2. 注册表优化 TCP 连接吞吐",
        "anchor": "#2-注册表优化-tcp-连接吞吐",
        "id": "2-注册表优化-tcp-连接吞吐"
      },
      {
        "title": "🛠️ 六、高频踩坑与排障自检清单",
        "anchor": "#六-高频踩坑与排障自检清单",
        "id": "六-高频踩坑与排障自检清单"
      }
    ]
  },
  ...
  },
  {
    "id": "github-yt-dlp",
    "type": "github",
    "title": "yt-dlp (yt-dlp/yt-dlp)",
    "url": "nav.html#github-yt-dlp",
    "externalUrl": "https://github.com/yt-dlp/yt-dlp",
    "category": "GitHub 导航 · 媒体与下载工具",
    "date": "2026-09-13",
    "tags": [
      "youtube",
      "下载",
    ...
    "summary": "功能强大的命令行音视频下载工具 — yt-dlp 是 youtube-dl 的一个分支，支持下载数千个网站的音视频，拥有强大的功能和持续的维护更新。",
    "content": "yt-dlp yt-dlp/yt-dlp https://github.com/yt-dlp/yt-dlp 功能强大的命令行音视频下载工具 yt-dlp 是 youtube-dl 的一个分支，支持下载数千个网站的音视频，拥有强大的功能和持续的维护更新。 youtube 下载 多媒体 命令行",
    "sections": []
  },
  {
    "id": "github-kugoumusic-net",
    "type": "github",
    "title": "KugouMusic.NET (Linsxyx/KugouMusic.NET)",
    "url": "nav.html#github-kugoumusic-net",
    "externalUrl": "https://github.com/Linsxyx/KugouMusic.NET",
    "category": "GitHub 导航 · 媒体与下载工具",
    "date": "2026-09-13",
    "tags": [
      "音乐",
    ...
    "summary": "第三方跨平台酷狗音乐客户端 — 基于 .NET 打造的第三方酷狗音乐客户端，无广告，提供清爽的听歌体验。",
    "content": "KugouMusic.NET Linsxyx/KugouMusic.NET https://github.com/Linsxyx/KugouMusic.NET 第三方跨平台酷狗音乐客户端 基于 .NET 打造的第三方酷狗音乐客户端，无广告，提供清爽的听歌体验。 音乐 酷狗 播放器 .NET",
    "sections": []
  },
  {
    "id": "github-landrop",
    "type": "github",
    "title": "LANDrop (LANDrop/LANDrop/releases)",
    "url": "nav.html#github-landrop",
    "externalUrl": "https://github.com/LANDrop/LANDrop/releases",
    "category": "GitHub 导航 · 网络辅助与传输工具",
    "date": "2026-09-13",
    "tags": [
      "局域网",
      "传输",
    ...
    "summary": "局域网跨平台文件互传工具 — LANDrop 可以在局域网内实现不同平台设备间的高速文件互传，即用即走。",
    "content": "LANDrop LANDrop/LANDrop/releases https://github.com/LANDrop/LANDrop/releases 局域网跨平台文件互传工具 LANDrop 可以在局域网内实现不同平台设备间的高速文件互传，即用即走。 局域网 传输 跨平台 效率",
    "sections": []
  },
  {
  ...
  },
  {
    ...
    "summary": "Cloudflare DDNS 动态域名解析自动检测与同步脚本（支持 IPv4/IPv6、缓存与定时任务） (7.1 KB, undefined 行)",
    "content": "cf-ddns.sh Cloudflare DDNS 动态域名解析自动检测与同步脚本（支持 IPv4/IPv6、缓存与定时任务） Shell 脚本 sh",
    "sections": []
  },
  {
    "id": "file-frps-sh",
